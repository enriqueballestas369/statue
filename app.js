...
async function loadPdf(file) {
  ...
  try {
    ...
    // NO_TEXT isn't really a failure to load — the PDF opened fine, it
    // just has no extractable text anywhere (typically a scanned image).
    // Show it like a normal (empty-text) document, with a clear banner,
    // instead of blocking the reader entirely.
    if (err && err.code === "NO_TEXT" && err.pages) {
      state.numPages = err.numPages;
      state.pages = err.pages.map((p) => ({
        pageNum: p.pageNum,
        paragraphs: [],
        pageError: false
      }));
      renderDocument({ noTextBanner: err.message });
      renderGuide();
      return;
    }
...
const ERROR_TITLES = {
  LIB_INIT_FAILED: "PDF reader couldn't start",
  INVALID_PDF: "This file isn't a readable PDF",
  PASSWORD_PROTECTED: "This PDF is password-protected",
  RENDER_FAILED: "This PDF couldn't be read",
  DEFAULT: "Could not load document"
};

function renderDocError(err) {
  const code = err && err.code;
  const title = ERROR_TITLES[code] || ERROR_TITLES.DEFAULT;
  const msg = (err && err.message) || "Could not read this PDF.";
  emptyState.style.display = "none";
  docContent.style.display = "block";
  docNameWrap.style.display = "none";
  pageNav.style.display = "none";
  docContent.innerHTML = `<div class="doc-kicker">${escapeHtml(title)}</div>
    <p style="font-family:var(--sans); color:var(--danger); font-size:14px; line-height:1.6;">${escapeHtml(msg)}</p>
    <button class="btn secondary" id="retryUploadBtn" style="margin-top:10px;">Choose a different PDF</button>`;
...
function renderDocument(opts = {}) {
  ...
  if (opts.noTextBanner) {
    html += `<div style="font-family:var(--sans); font-size:13px; color:#8A6D1F; background:#F3EEDD; border-radius:5px; padding:11px 13px; margin-bottom:24px; line-height:1.55;">${escapeHtml(opts.noTextBanner)}</div>`;
  }
  ...
  state.pages.forEach((p) => {
    ...
    if (p.pageError) {
      html += `<div class="doc-page-empty">This page couldn't be read individually, but the rest of the document loaded fine.</div>`;
    } else if (p.paragraphs.length === 0) {
