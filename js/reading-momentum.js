(() => {
  ...
  function buildMissionStages(){
    ...
    const chosen = missions.slice(0, count);
    return chosen.map((mission, i) => {
  ...
  function buildStages(){
    ...
    // Structural labels are useful orientation, but not enough to measure reading progress.
    // If the extractor only exposes one or two reliable headings, use neutral reading missions
    // across the whole document rather than pretending the whole document is one legal section.
    if (structural.length < 3) {
      stages = buildMissionStages();
      return;
    }
    ...
    stages = structural.map((item, i) => {
      ...
      return {
        label: item.label,
        task: item.label === "BACKGROUND / FACTS" ? "Understand the setting without treating every detail as equally important." :
              item.label === "RULE / LEGAL STANDARD" ? "Identify the governing law or standard." :
              item.label === "REASONING / ANALYSIS" ? "Slow down and follow how the court applies the law." :
              item.label.includes("REMEDY") ? "Separate the legal decision from the action the court orders." :
              `Read this section for the job it performs in the document.`,
        start:startPage,
        end:endPage,
        el:item.el
      };
  ...
  function render(){
    ...
    bar.addEventListener("click",e=>{
      ...
      if(b.dataset.act==="resume"){
        const p=bar.querySelector(".momentum-resume"); p.hidden=!p.hidden; if(!p.hidden)p.querySelector("textarea").focus();
      }
      if(b.dataset.act==="save"){
        setSession({docKey:docKey(),stage:currentStage,note:bar.querySelector("textarea").value});
        bar.querySelector(".momentum-resume").hidden=true;
      }
      if(b.dataset.act==="focus"){
        document.body.classList.toggle("reading-focus");
        b.textContent=document.body.classList.contains("reading-focus")?"Show full document":"Focus on this mission";
        markCurrentPages();
      }
    });
    updateBar();
  ...
  function markCurrentPages(){
    const pages=[...root.querySelectorAll(".doc-page")];
    pages.forEach(p=>p.classList.remove("momentum-current-page"));
    const s=stages[currentStage]; if(!s)return;
    for(let i=s.start;i<=s.end;i++) pages[i]?.classList.add("momentum-current-page");
  }

  function updateBar(){
    const bar=document.querySelector(".reading-momentum"); if(!bar||!stages.length)return;
    const s=stages[currentStage];
    bar.querySelector(".momentum-stage").textContent=s.label;
    bar.querySelector(".momentum-task").textContent=s.task;
    bar.querySelector(".momentum-progress").textContent=`Mission ${currentStage+1} of ${stages.length}`;
    bar.querySelector(".momentum-fill").style.width=`${((currentStage+1)/stages.length)*100}%`;
    const pages=[...root.querySelectorAll(".doc-page")];
    bar.querySelector(".momentum-range").textContent = pages.length > 1 ? `Current reading area: pages ${s.start+1}${s.end>s.start?`–${s.end+1}`:""}` : "";
    markCurrentPages();
  }

  function observeReading(){
    const pane=document.getElementById("docPane"); if(!pane||pane.dataset.momentumScroll)return;
    pane.dataset.momentumScroll="true";
    pane.addEventListener("scroll",()=>{
      const pages=[...root.querySelectorAll(".doc-page")]; if(!pages.length||!stages.length)return;
      const line=pane.getBoundingClientRect().top+150;
      let pageIndex=0;
      for(let i=0;i<pages.length;i++){ if(pages[i].getBoundingClientRect().top<=line) pageIndex=i; else break; }
      const next=stages.findIndex(s=>pageIndex>=s.start && pageIndex<=s.end);
      if(next>=0 && next!==currentStage){
        currentStage=next; updateBar();
        setSession({docKey:docKey(),stage:currentStage,note:document.querySelector(".momentum-resume textarea")?.value||""});
      }
    },{passive:true});
  }

  function init(){
    css(); root=document.querySelector("#docContent .doc-body");
    if(!root||!root.innerText.trim())return false;
    if(root.dataset.momentumReady)return true;
    if(!document.querySelector(".learning-map"))return false;
    root.dataset.momentumReady="true";
    buildStages();
    const saved=getSession();
    if(saved?.docKey===docKey() && Number.isInteger(saved.stage)) currentStage=Math.min(saved.stage,stages.length-1);
    render(); observeReading();
    if(saved?.docKey===docKey() && saved.stage>0){
      setTimeout(()=>{stages[currentStage]?.el?.scrollIntoView({block:"start"}); updateBar();},80);
    }
    return true;
  }
